...
### 🔴 7. POBJ writer の attribute format builder ✅ 完了
...
- 内部 `encode_pos` / `encode_normal` / `encode_color` / `encode_uv` ヘルパで (バッファ生成 + AttrLayout {CompCnt, CompType, Scale, Stride}) を一手に算出。`AttrSpec` はここから値を受け取り、writer は `set_u8(off+0x10, scale)` で GX `Scale` field を書き込む。
- PyO3 binding に `MeshBuilder.set_pos_format(fmt[, *, exponent])` 等を expose。

**Stage 2 — 非 default variant 本体実装**:
- `S16x3 { exponent }` / `S8x3 { exponent }` (POS / NRM):
  - 符号付き量子化 `round(value × (1 << exponent))` → `i16` / `i8` clamp
  - reader は GX `Scale` から `1 / (1 << scale)` 復元、round-trip 完成
  - `comp_type = GxCompType::Int16(3) / Int8(1)`、`stride = 6 / 3`
- `S16x2 { exponent }` / `S8x2 { exponent }` (TEX0): 同上、`stride = 4 / 2`
- `Rgb565` (CLR0): HSDLib quirk bit layout (high 5 = b, mid 6 = g, low 5 = r)、`comp_type = 0`、`comp_count = 0 (RGB)`、`stride = 2`
- `Rgba4` (CLR0): 各 channel 上位 4 bits、byte0 = (R<<4 | G), byte1 = (B<<4 | A)、`comp_type = 3`、`comp_count = 1 (RGBA)`、`stride = 2`

**Stage 3 — テスト**:
- `format_setters_with_defaults_match_no_setter_path` (= default で setter 呼び出しが setter 無しと byte-identical)
- `pos_format_s16x3_round_trips_via_attribute_scale` (exponent=8 量子化、1 LSB ≈ 1/256)
- `pos_format_s8x3_round_trips_via_attribute_scale` (exponent=4 量子化、1 LSB ≈ 1/16)
- `uv_format_s16x2_round_trips_via_attribute_scale` (exponent=12 UV 空間、1 LSB ≈ 1/4096)
- `color_format_rgb565_round_trips` (5-bit R/B、6-bit G、primary color stays dominant)
- `color_format_rgba4_round_trips` (4-bit per channel、各 channel 上位 4 bits round-trip)

= 全 6 件 pass。pobj_writer 26 件 total。

設計上残しているもの:
- `U16x3` / `U8x3` 等の unsigned variant — HSDLib `GXCompType::UInt8/UInt16` 経路。consumer 要望が来てから追加。
- vertex-cache-aware triangle stripping (現状 greedy) — `todo.md` §3 で別 issue 扱い。

### 🔴 8. paletted format encoder ✅ 完了

現状 (本タスク前): CI4 / CI8 / CI14X2 / I4 / I8 / IA4 / IA8 が read-only。

実装内容 (本タスク完了分):

**Stage 1 — intensity 系 (I4 / I8 / IA4 / IA8)**:
- `gx_image::encode_image` の format match に追加 (palette 不要)
- BT.601 luminance `Y = 0.299*R + 0.587*G + 0.114*B` を採用 (HSDLib decoder は one-way なので公式定数を採用)
- 各 format で `from_*` decoder の inverse を実装、tile pack (I4: 8×8 / I8: 4×8 / IA4: 4×8 / IA8: 4×4)
- テスト 4 件 (`i4_round_trip_grayscale` / `i8_round_trip_grayscale` / `ia4_round_trip_with_alpha` / `ia8_round_trip_with_alpha_lossless_for_grayscale`) で grayscale + alpha の round-trip + RMS budget を gate

**Stage 2 — paletted 系 (CI4 / CI8 / CI14X2)**:
- 新 API: `gx_image::encode_paletted_image(format, w, h, rgba, tlut_format, max_palette) -> EncodedPaletted { image, palette_rgba, palette_bytes }`
- 既存 `encode_image` の CI4 / CI8 / CI14X2 arm は「TLUT 不在なので `encode_paletted_image` を使え」と error
- palette 量子化: median-cut algorithm (= 各 bucket を最大 channel range の axis で split、centroid を palette entry に)。RGBA 4D 空間で動作。
- unique-color fast path: `unique_count <= k` なら median-cut スキップ
- nearest-color lookup: 線形 scan、squared RGBA distance + HashMap キャッシュで O(N*K) を緩和
- tile pack: `pack_ci4` (8×8 block、1 nibble/pixel) / `pack_ci8` (4×8 block、1 byte/pixel) / `pack_ci14x2` (4×4 block、2 bytes BE、低 14 bits = index)
- TLUT bytes encoder: `palette_to_tlut_bytes(palette, GxTlutFmt)` — IA8 / RGB565 / RGB5A3 の wire format (2 bytes BE/entry)。既存 `encode_rgb5a3_pixel` を helper として抽出して再利用。

**Stage 3 — テスト (各 round-trip で TLUT 経由 decode)**:
- `ci4_round_trip_with_unique_palette` (8 unique colors + CI4 16-entry palette + RGB5A3 lossless for primary colors → byte-exact)
- `ci8_round_trip_via_median_cut` (256 distinct colors + 64-entry palette + RMS < 32)
- `ci14x2_round_trip_lossless_for_small_palette` (8 colors + CI14X2 + RGB5A3 → byte-exact)
- `paletted_rejects_non_paletted_format`
- `encode_rejects_paletted_formats_via_unpaletted_path`

= gx_encoder 全 21 件 pass。

**Stage 4 — PyO3 binding**:
- 新 module function `gx_encode_paletted(format, w, h, rgba, /, tlut_format=2, max_palette=None) -> (image_bytes, palette_bytes, palette_rgba_flat)`
- 既存 `gx_encode` docstring は intensity formats を含め updateされ、paletted 系は `gx_encode_paletted` を案内
...
paletted encode 経由でも、現在の `MeshBuilder` / `Image` / `TObj` 経路は `tobj.set_image_data(image)` + `tobj.set_tlut_data(tlut)` の 2 段で配線する。`encode_paletted_image` の戻り値 `(image_bytes, palette_bytes, palette_rgba)` を `Image.set_image_data_bytes(image_bytes)` と `Tlut.set_palette_bytes(palette_bytes)` にそれぞれ渡す形。`MObj::allocate_textured_with` の docstring 既に「TLUT は別途 attach 必要」と明示済 (= #3 で softening した内容)。
...
1. **🔴 1 / 2 / 3 を 1 commit でまとめる** ✅ — README / module doc / docstring 文言差し替え (de-couple #1-8 commit)
2. **🔴 4 / 5** ✅ — preset の generic kwargs 化 + `alloc_scene_data` rename (同上)
3. **🔴 6** ✅ — test 経路の rename と分離。CI workflow も合わせて修正
4. **🔴 7** ✅ — POBJ writer attr format builder の本体実装 (本 commit)
5. **🔴 8** ✅ — paletted encoder の本体実装 (本 commit)
6. **⚪ 9** — fighter / MEX / kex 互換 flag 実装。要望待ち

ベンチマーク的には 1〜3 が即直し、4〜6 が API breaking refactor、7〜8 が新機能拡張、9 は requested-only。1-8 完了。
